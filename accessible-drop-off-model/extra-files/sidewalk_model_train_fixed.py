...
def aggregate_ratings_fixed(image_sel_csv, mobility_classes):
    """
    FIXED: Properly interpret the selection data.
    ...
    # FIXED: Invert the mapping - Yes = accessible (1.0), No = inaccessible (0.0)
    ...
    pivot = df.groupby(["ImageID", "MobilityAid"])["accessibility_score"].mean().unstack(fill_value=0.5)

    # Ensure all target classes exist
    for cls in mobility_classes:
        if cls not in pivot.columns:
            pivot[cls] = 0.5  # Default to neutral if no data

    # Merge potential duplicates
    pivot.columns = [normalize_mobility_name(c) for c in pivot.columns]
    pivot = pivot.T.groupby(level=0).mean().T  # Use mean instead of sum
...
def train_model():
    ...
    aggregated_df.to_csv("Project_Sidewalk_Data/aggregated_fixed.csv", index=False)
    ...
    train_loader, val_loader = create_data_loaders(
        "Project_Sidewalk_Data/aggregated_fixed.csv",
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, "accessibility_checkpoint_fixed.pth")
