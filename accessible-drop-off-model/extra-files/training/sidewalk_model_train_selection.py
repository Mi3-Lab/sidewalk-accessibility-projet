...
def aggregate_ratings_fixed(image_sel_csv, mobility_classes):
    """
    'Yes' means the sidewalk IS accessible for that mobility aid (low difficulty)
    'No' means the sidewalk is NOT accessible (high difficulty)
    'Unsure' is neutral
    
    We want to predict ACCESSIBILITY scores (higher = more accessible)
    """
    df = pd.read_csv(image_sel_csv)
    
    # Invert the mapping - Yes = accessible (1.0), No = inaccessible (0.0) 
    # TODO: Try yes = 1, No = -1, Unsure = 0.0, use Hyperbolic Tangent 
    selection_map = {"Yes": 1.0, "Unsure": 0.5, "No": 0.0}
    df["accessibility_score"] = df["Selection"].map(selection_map)

    # Normalize MobilityAid names before grouping
    df["MobilityAid"] = df["MobilityAid"].apply(normalize_mobility_name)

    # Group by ImageID and MobilityAid, take mean of accessibility scores
    pivot = df.groupby(["ImageID", "MobilityAid"])["accessibility_score"].mean().unstack(fill_value=0.25)
    # Better to have a lower accessibility score for missing data than 0? so default to 0.25

    # Merge duplicates, was having issues after unstacking, so added this
...
def train_model():
    ...
    # First, regenerate the aggregated data with fixed scoring
    print("Regenerating aggregated data with fixed scoring...")
    aggregated_df = aggregate_ratings_fixed("Project_Sidewalk_Data/image_selection.csv", mobility_classes)
    aggregated_df.to_csv("Project_Sidewalk_Data/aggregated_yes_no.csv", index=False)
    ...
    train_loader, val_loader = create_data_loaders(
        "Project_Sidewalk_Data/aggregated_yes_no.csv",
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            }, "accessibility_checkpoint_select.pth")
