...
async function submitToServer() {
  ...
  if (state.submitted_at) {
    if (statusEl) statusEl.textContent = "Responses already submitted.";
    return;
  }
  if (statusEl) statusEl.textContent = "Submitting responses...";
  const payload = exportPayload();
  const formData = new URLSearchParams();
  formData.set("form-name", "accessibility-study");
  formData.set("session_id", state.session_id);
  formData.set("participant_id", state.participant_id);
  formData.set("primary_aid", state.profile.primary_aid || "");
  ...
  try {
    ...
    if (res.ok) {
      ...
      saveState();
      if (statusEl) statusEl.textContent = "Responses submitted. You may also download a backup below.";
    } else {
      if (statusEl) statusEl.textContent = "Submit failed. Please download your data below and contact the study team.";
    }
  } catch {
    if (statusEl) statusEl.textContent = "Offline. Please download your data below and contact the study team.";
  }
}
