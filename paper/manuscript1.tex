...
\title{Human-Aligned Sidewalk Accessibility Perception: Soft-Label Learning from 829 Mobility Aid Users for Autonomous Vehicle Mobility Services}
...
\begin{abstract}
Before a shared autonomous vehicle opens its door at a curbside drop-off location, the planner must answer the question if the adjacent sidewalk is accessible for the passenger’s specific mobility aid. Rather than collapsing the problem into binary labels, we instead frame the issue as calibrated perception for risk-aware pedestrian routing. Using $49{,}490$ ternary responses (yes/unsure/no) from $829$ mobility-aid users on $52$ Google Street View scenes via Project Sidewalk, it yields an empirical per-aid vote distributions that we use as the operational training target A frozen vision encoder with a per-aid linear probe trained by KL divergence against these distributions achieves a soft Brier score of $0.076$ (mean across eight backbones). This results in a $\mathbf{3.2\times}$ calibration improvement over standard cross-entropy that holds for every encoder tested. To demonstrate this, we integrate these calibrated probabilities into an OpenStreetMap graph to power a risk-aware routing algorithm. The planner effectively trades a minor distance overhead ($7.4$--$9.8\%$) for safer drop-off locations, with model-predicted passability improving on $87.8$--$91.0\%$ of trips. In a $50$-pair head-to-head comparison, our approach achieves higher model-predicted passability than OpenRouteService's (ORS) standard wheelchair profile on all five aids, and successfully maps the $20\%$ of routes where ORS fails to find any path at all. We frame this module as a learned map-prior layer for AV drop-off planning, seamlessly integrating with onboard camera/LiDAR verification at deployment. It runs at just 78.5\,ms/image on an A100 GPU, ${\sim}75\times$ faster than the closest zero-shot VLM.
\end{abstract}
...
A shared autonomous vehicle preparing for a curbside drop-off must answer a question the onboard map almost never resolves: \emph{is the sidewalk segment between the door and the passenger's destination passable for that passenger's specific level of mobility?} This last-meter perception problem~\citep{bolten2017accessmap,saha2022mypath,tannert2018wheelchair} sits between two well-studied layers of the autonomy stack: the geometric perception running onboard~\citep{Saha_ProjectSidewalk_CHI2019,o'meara2025rampnet,hosseini2023tile2net} and the rule-based pedestrian-routing engine that consumes city-scale maps~\citep{openrouteservice2026wheelchair,bolten2019opensidewalks}. The issue is that neither of these existing systems is calibrated for the actual risk experienced by a mobility-aid user. Current deployed systems lack calibrated per-aid probability models for sidewalk passability; curb selection remains generic rather than personalized to mobility aid. What one manual-wheelchair user judges traversable, another may judge impassable for entirely correct reasons~\citep{DBLP:journals/corr/abs-2502-19888}. If a navigation planner receives this data as a continuous probability rather than a simple binary ``yes/no'' label, it can assess whether it should trade a shorter, riskier path for a longer, safer one. Therefore, the core robot-learning question is not ``can a network classify sidewalks,'' but \emph{can a learned perception module generate a calibrated, per-aid probability that an autonomous planner can reliably use to navigate uncertainty?}
...
% \textbf{Our position.} 
We treat the empirical vote distribution $[p_{\mathrm{no}}, p_{\mathrm{unsure}}, p_{\mathrm{yes}}]$ over $\{$no, unsure, yes$\}$ as the operational reference signal for risk-aware autonomous mobility services (Fig.~\ref{fig:teaser_b}). With $\approx 190$ independent votes per (image, aid) pair, nearly four times the per-image annotation density of CIFAR-10H~\citep{peterson2019humanuncertainty} and far richer than ImageNet-16H~\citep{steyvers2022bayesian}, each distribution is a statistically reliable empirical estimate of community-level opinion, not a noisy majority vote from three crowd-workers. A frozen vision encoder + per-aid linear probe trained against these distributions by KL divergence produces a calibrated per-aid probability that we drop directly into an OpenStreetMap (OSM)-derived Pittsburgh pedestrian graph as edge cost. The planner solves a single risk-aware Dijkstra and produces five \emph{different} optimal paths for the same origin-destination (OD) pair, one per aid (Fig.~\ref{fig:teaser}c). The metric that matters for this pipeline is calibration (soft Brier score), \emph{not} argmax F1: the planner's edge weight is a linear function of $\hat{p}_\mathrm{yes}$, so an overconfident probe can achieve high F1 while still collapsing the routing gradient.


\textbf{Robot-learning interface.} The module is a learned map-prior layer between candidate drop-off generation and final stop selection, not a replacement for onboard perception. Offline Street-View imagery and Project Sidewalk votes assign per-aid edge priors; at request time, the planner queries the passenger's aid, ranks curb stops by downstream sidewalk confidence, and leaves transient hazards (parked vehicles, snow, construction) to live camera/LiDAR verification. At $78.5$\,ms/image on an A100, DINOv2-large is ${\sim}75\times$ faster than the closest zero-shot VLM.
...
\textbf{(C4)} We conduct an encoder-transfer probe in Detroit, Zurich, and Taipei, which identifies a Western-centric data limitation (Taipei drops near chance) and motivates a deployment safeguard: reject the map-prior in regions where transfer accuracy is unreliable.
...
\textbf{Automated accessibility perception and pedestrian routing.} Prior work crowdsources Street-View sidewalk audits~\citep{10.1145/2470654.2470744,10.1145/2642918.2647403,10.1145/3308561.3353798,Saha_ProjectSidewalk_CHI2019}, scales them with classifiers~\citep{o'meara2025rampnet,10.1145/3663548.3688531}, and complements them with aerial extraction~\citep{hosseini2023tile2net,hosseini2022citysurfaces} and multimodal sensing~\citep{damaceno2024sideseeing,10.1145/3441852.3476560}. Routing systems such as OpenRouteService~\citep{openrouteservice2026wheelchair}, AccessMap/OpenSidewalks~\citep{bolten2017accessmap,bolten2019opensidewalks}, MyPath~\citep{saha2022mypath}, and Tannert and Sch{\"o}ning~\citep{tannert2018wheelchair} consume sparse accessibility metadata or user weights. In contrast, we learn calibrated per-aid probabilities from mobility-aid users' own vote distributions and use them as Dijkstra edge costs, covering 91.7\% of edges where ORS-style lookup leaves 20\% of OD pairs unroutable.
...
\textbf{Soft labels and calibration for safety-critical learning.} Label Distribution Learning~\citep{geng2016label}, CIFAR-10H~\citep{peterson2019humanuncertainty}, ImageNet-16H~\citep{steyvers2022bayesian}, sparse-annotator learning~\citep{collins2022elicit}, crowd-distance prediction~\citep{jiang2024crowdcalibrator}, and broad soft-label benchmarks~\citep{xia2025softlabels} establish empirical distributions as training targets. Unlike distillation~\citep{hinton2015distilling}, our distributions come from humans, not a teacher model. Calibration work shows that modern networks are overconfident on subjective inputs~\citep{guo2017calibration,minderer2021revisiting} and that embodied agents should optimise the probability scale consumed by downstream planners~\citep{zollo2025calibration}; our soft Brier metric follows from that planner interface.
...
\textbf{Why a small visual set is appropriate.} The probe (\S\ref{sec:method}) has only $3(d{+}1)$ parameters per aid ($d=1024 \Rightarrow 3075$ for DINOv2-large) and is fit against $52$ vector-valued targets per aid; each target component is the mean of $\sim$60--70 Bernoulli draws (binomial $95\%$ CI $\pm 0.06$--$0.13$). Pixel-level features come from foundation encoders pretrained on $10^8$--$10^9$ images~\citep{radford2021learningtransferablevisualmodels,oquab2023dinov2,tschannen2025siglip2}; the probe learns an aid-conditioned linear projection onto the 3-simplex. The modest visual sample size is compensated by label density: each of the $52$ scenes is annotated by real mobility-aid users giving authentic feedback under real mobility constraints, not synthetic or crowd-sourced binary labels. The calibration claim is therefore local to the support of these $52$ scenes---we treat the visual sample size as the headline limitation in \S\ref{sec:limitations}. Walking cane shows the strongest yes-skew (45/52 majority-yes, 6.4:1) and motorised wheelchair is evenly split (26/26); the imbalance asymmetry drives the per-aid behaviour analysed in \S\ref{sec:results}.

\FloatBarrier
\section{Method}
\label{sec:method}

A per-aid linear probe on a frozen encoder is trained to match the empirical ternary vote distribution $[p_{\mathrm{no}}, p_{\mathrm{unsure}}, p_{\mathrm{yes}}]$ via KL divergence (Soft-KL) and evaluated against a hard cross-entropy baseline; calibration is measured by soft Brier score under 5-fold panorama-level cross-validation.
...
All numbers below are 5-fold panorama-level CV at seed $42$; $\pm$ values are 95\% CIs over the $40$ (fold, aid) measurements per encoder. The claim is not visual foundation-model learning but whether frozen urban-scene features can be calibrated against dense human vote distributions to produce a probability an autonomous driving planner can consume.

\textbf{Soft-KL vs.\ Hard-CE calibration.} Table~\ref{tab:results_all} and Fig.~\ref{fig:brier} (Appendix~\ref{app:calibration}) report the primary calibration result alongside argmax F1. Soft-KL achieves a soft Brier score of $0.064$--$0.083$ versus $0.228$--$0.274$ for Hard-CE---a \textbf{$3.2\times$ mean improvement} consistent across every backbone and outside the 95\% CI for all eight pairings. Post-hoc temperature scaling (NLL-tuned per fold; Appendix~\ref{app:calibration}, Table~\ref{tab:temp_scaling}) reduces Hard-CE Brier by ${\approx}34\%$ on average but leaves a $>2\times$ gap vs Soft-KL, confirming the advantage is structural (target encoding) rather than overconfidence alone. At $\beta_c{=}8$, Hard-CE's overconfident edge probabilities collapse the planner's routing gradient; Soft-KL preserves the empirical bimodality and gives the planner a usable continuous risk signal.

\begin{table}
\caption{\textbf{Per-encoder results, Soft-KL (ours) vs.\ Hard-CE baseline.} 5-fold panorama-level CV at seed $42$; $\pm$ is 95\% CI over $40$ fold-aid measurements. Brier $\downarrow$ is measured against human vote distributions; zero-shot VLMs use no task-specific data.}
...
\centering
\caption{\textbf{Comparison with ORS} on the 40/50 routable Pittsburgh pairs. Our scoring yields higher mean $\hat{p}_\mathrm{yes}$ on every aid.}
...
\textbf{Calibration is the metric the robot planner selects.} The Dijkstra objective is linear in $\hat{p}_\mathrm{yes}$, so Hard-CE's overconfidence~\citep{guo2017calibration,minderer2021revisiting} directly distorts routing costs. A borderline edge with $62\%$ yes-votes receives $\hat{p}_\mathrm{yes}\approx0.97$ under Hard-CE but $\hat{p}_\mathrm{yes}\approx0.65$ under Soft-KL, collapsing the planner's contrast signal across $91.7\%$ of Pittsburgh edges. This is why Soft-KL routing improves model-predicted passability on $88$--$91\%$ of random OD trips, consistent with calibration-aware deployment guidance for embodied agents~\citep{zollo2025calibration}.
...
\textbf{Visual sample and statistical scope.} Training uses 52 GSV views from nine cities ($3{,}075$ probe parameters per aid for DINOv2-large). The calibration claim rests on the $3.2\times$ Brier ratio between objectives, not absolute F1; encoder rankings within overlapping CIs are suggestive rather than definitive.

\textbf{Geographic and demographic coverage.} Taipei transfer is near chance ($0.518$), so Western-centric data is the headline limitation. Demographics beyond aid type were not collected, and aid classes conflate equipment geometry, motor power, and individual risk tolerance. A deployed map-prior should be suppressed in regions where transfer balanced accuracy is near chance (e.g., $\leq0.55$) until additional local audits validate it.

\textbf{Single routing city, offline evaluation, and evaluation circularity.} Routing uses one Pittsburgh OSM snapshot; the ORS comparison is $n{=}50$ pairs and directional. We have not run a live wheelchair-user study, evaluated transient obstructions or sensor noise, or tested low-power deployment. Because the routing metric is model-predicted $\hat{p}_\mathrm{yes}$, the $87.8$--$91.0\%$ gain shows the planner optimises its objective; proving actual user safety requires held-out Pittsburgh audits, route-preference studies, or physical traversal measurements.

\textbf{Responsible deployment.} The per-aid probability is a community-level prior, not a per-user safety guarantee. A deployed system should use it only for route ranking, verify curbside scenes with onboard sensors~\citep{saha2022mypath}, expose uncertainty, reject near-chance regions, and confirm candidate routes with the passenger before committing to a path.

\section{Conclusion}
\label{sec:conclusion}

We presented a learned perception module that treats vote distributions from 829 mobility-aid users as first-class training targets and produces calibrated per-aid passability probabilities for autonomous mobility planning. A frozen-encoder probe trained by KL divergence achieves a $3.2\times$ soft-Brier improvement over hard cross-entropy across eight backbones; composed into risk-aware Dijkstra, it achieves model-predicted passability gains on $87.8$--$91.0\%$ of $10{,}000$ Pittsburgh OD pairs at $7.4$--$9.8\%$ distance overhead and yields higher model-predicted passability than OpenRouteService's wheelchair profile on every tested aid. The central lesson is that when subjective human judgment supervises a downstream policy, calibration---not argmax accuracy---is the operative metric. Code and pre-extracted features will be released upon acceptance.
...
\centering
\begin{subfigure}[t]{0.32\linewidth}
\centering\vspace{0pt}\includegraphics[width=\linewidth]{images/route_walking_cane.png}
\caption{Walking cane: $\bar{p}_\mathrm{yes} = 0.76$, $+2.8\%$ dist.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\linewidth}
\centering\vspace{0pt}\includegraphics[width=\linewidth]{images/route_walker.png}
\caption{Walker: $\bar{p}_\mathrm{yes} = 0.70$, $+9.0\%$ dist.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\linewidth}
\centering\vspace{0pt}\includegraphics[width=\linewidth]{images/route_mobility_scooter.png}
\caption{Mobility scooter: $\bar{p}_\mathrm{yes} = 0.64$, $+12.9\%$ dist.}
\end{subfigure}\\[0.5em]
\begin{subfigure}[t]{0.32\linewidth}
\centering\vspace{0pt}\includegraphics[width=\linewidth]{images/route_manual_wheelchair.png}
\caption{Manual wheelchair: $\bar{p}_\mathrm{yes} = 0.57$, $+14.5\%$ dist.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.32\linewidth}
\centering\vspace{0pt}\includegraphics[width=\linewidth]{images/route_motorized_wheelchair.png}
