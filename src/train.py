...
def train_aid_model(model, processor, device, df_aid, output_dir, aid, model_name, model_type="clip"):
    # Split dataframe first for fair comparison across models
    tr_df, va_df = stratified_split_min1(df_aid, label_col='argmax_label', test_size=0.2, random_state=42)
    
    # Extract features for train
    train_paths = tr_df['path'].tolist()
    if model_type == "clip":
        X_train = extract_features(model, processor, device, train_paths)
    elif model_type in ["resnet50", "vit_base_patch16_224", "convnext_base"]:
        X_train = extract_vision_features(model, processor, device, train_paths)
    else:
        raise ValueError("Unknown model_type")
    
    # Extract features for val
    val_paths = va_df['path'].tolist()
    if model_type == "clip":
        X_val = extract_features(model, processor, device, val_paths)
    elif model_type in ["resnet50", "vit_base_patch16_224", "convnext_base"]:
        X_val = extract_vision_features(model, processor, device, val_paths)
    else:
        raise ValueError("Unknown model_type")
    ...
    w_val = va_df['sample_weight'].values
    
    scaler = StandardScaler(with_mean=False)
    X_train_s = scaler.fit_transform(X_train)
    X_val_s = scaler.transform(X_val)
    
    # Use a linear layer as probe, fine-tune it
    import torch.nn as nn
    import torch.optim as optim
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    probe = nn.Linear(X_train_s.shape[1], 3).to(device)
    optimizer = optim.Adam(probe.parameters(), lr=0.001)
    criterion = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 1.0, 1.0], device=device))  # adjust weights if needed
    ...
    meta = {
        ...
        "classes_seen": [0,1,2]  # since probe has 3 outputs
    }
